...
export function handleTransfer(event: Transfer): void {
  ...
  if (fromAddress.toHex() == ZERO_ADDRESS) {
    ...
    if ( !(excludedAddresses.includes(toAddress.toHex()) || excludedAddresses.includes(fromAddress.toHex())) ) { 
      let sourceAccountPosition = loadOrCreatePosition(fromAddress, event.address.toHex());
      sourceAccountPosition.netAmount =  sourceAccountPosition.netAmount.minus( amountLP )
      sourceAccountPosition.longAmount =  sourceAccountPosition.longAmount.minus( amountLP )
      

      let destinationAccountPosition = loadOrCreatePosition(toAddress, event.address.toHex());
      destinationAccountPosition.netAmount =  destinationAccountPosition.netAmount.plus( amountLP )
      destinationAccountPosition.longAmount =  destinationAccountPosition.longAmount.plus( amountLP )
      ...
      const sourceAccountOptionsTransferTransactions = sourceAccountPosition.optionsTransferTransactions
      sourceAccountOptionsTransferTransactions.push( event.transaction.hash.toHex() )
      sourceAccountPosition.optionsTransferTransactions = sourceAccountOptionsTransferTransactions   

      const destinationAccountOptionsTransferTransactions = destinationAccountPosition.optionsTransferTransactions
      destinationAccountOptionsTransferTransactions.push( event.transaction.hash.toHex() )
