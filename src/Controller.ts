...
export function handleCollateralAssetDeposited(event: CollateralAssetDeposited): void {
  ...
  if (vault) {
    ...
    vault.collateralAmount = collateralAmount ? collateralAmount.plus(amount) : BIGINT_ZERO.plus(amount)
    vault.save()
  }

  // create action entity
  let actionId = 'DEPOSIT-COLLATERAL-' + event.transaction.hash.toHex() + '-' + event.logIndex.toString()
...
export function handleCollateralAssetWithdrawed(event: CollateralAssetWithdrawed): void {
  ...
  // create action entity
  let actionId = 'WITHDRAW-COLLATERAL-' + event.transaction.hash.toHex() + '-' + event.logIndex.toString()
...
export function handleLongOtokenWithdrawed(event: LongOtokenWithdrawed): void {
  ...
  // create action entity
  let actionId = 'WITHDRAW-LONG-' + event.transaction.hash.toHex() + '-' + event.logIndex.toString()
  ...
  if (vault) {
    ...
    if (loss.gt(BIGINT_ZERO)) {
      if (action.collateral == USDC_ADDRESS) {
        ...
        if (otoken) {
          const expiryPrice = ExpiryPrice.load(WETH_ADDRESS + '-' + otoken.expiryTimestamp.toString())
          if (expiryPrice) {
            const wethPrice = expiryPrice.price // 8 decimals
            const usdcLoss = loss.times(wethPrice).div(BigInt.fromString('100000000000000000000'))
...
export function handleShortOtokenBurned(event: ShortOtokenBurned): void {
  ...
  // if amount = 0, set the longOtoken back to null
  vault.shortOToken = shortAmount && shortAmount.minus(amount).isZero() ? null : asset.toHex() // convert to id
